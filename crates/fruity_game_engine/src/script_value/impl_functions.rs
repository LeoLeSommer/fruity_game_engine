...
{
    fn into_script_value(self) -> FruityResult<ScriptValue> {
        Ok(ScriptValue::Callback(Box::new(|args: Vec<ScriptValue>| {
    ...
    for Box<dyn FnOnce() -> FruityResult<R> + Send + Sync + 'static>
{
    fn from_script_value(value: ScriptValue) -> FruityResult<Self> {
        let callback = <Box<dyn Fn() -> FruityResult<R> + Send + Sync>>::from_script_value(value)?;
...
impl<R: TryFromScriptValue> TryFromScriptValue for Box<dyn Send + Sync + Fn() -> FruityResult<R>> {
    fn from_script_value(value: ScriptValue) -> FruityResult<Self> {
        match value {
            ScriptValue::Callback(value) => Ok(Box::new(move || {
...
impl<
    ...
    > TryFromScriptValue
    for Box<dyn Send + Sync + Fn(T1, T2) -> Pin<Box<dyn Send + Future<Output = FruityResult<R>>>>>
{
    fn from_script_value(value: ScriptValue) -> FruityResult<Self> {
        match value {
            ScriptValue::Callback(value) => {
                ...
                Ok(Box::new(move |arg1: T1, arg2: T2| {
    ...
    fn from_script_value(value: ScriptValue) -> FruityResult<Self> {
        match value {
            ScriptValue::Callback(value) => {
                ...
                Ok(Arc::new(move |arg1: T1, arg2: T2| {
                    let shared_callback = shared_callback.clone();
                    Box::pin(async move {
                        let args: Vec<ScriptValue> =
                            vec![arg1.into_script_value()?, arg2.into_script_value()?];
...
impl<T1: TryIntoScriptValue, R: TryFromScriptValue> TryFromScriptValue
    for Box<dyn Send + Fn(T1) -> FruityResult<R>>
{
    fn from_script_value(value: ScriptValue) -> FruityResult<Self> {
        match value {
            ScriptValue::Callback(value) => Ok(Box::new(move |arg1| {
...
impl<T1: TryFromScriptValue + 'static, R: TryIntoScriptValue + 'static> TryIntoScriptValue
    for Box<dyn Send + Sync + Fn(T1) -> R>
{
    fn into_script_value(self) -> FruityResult<ScriptValue> {
        Ok(ScriptValue::Callback(Box::new(
...
impl<T1: TryIntoScriptValue, T2: TryIntoScriptValue, R: TryFromScriptValue> TryFromScriptValue
    for Box<dyn Send + Sync + Fn(T1, T2) -> FruityResult<R>>
{
    fn from_script_value(value: ScriptValue) -> FruityResult<Self> {
        match value {
            ScriptValue::Callback(value) => Ok(Box::new(move |arg1, arg2| {
                let args: Vec<ScriptValue> =
                    vec![arg1.into_script_value()?, arg2.into_script_value()?];
                let result = value(args)?;
...
impl<
    ...
    > TryIntoScriptValue for Box<dyn Send + Sync + Fn(T1, T2) -> R>
{
    fn into_script_value(self) -> FruityResult<ScriptValue> {
        Ok(ScriptValue::Callback(Box::new(
            Box::new(move |args: Vec<ScriptValue>| {
...
impl<
        ...
        R: TryFromScriptValue,
    > TryFromScriptValue for Box<dyn Send + Fn(T1, T2, T3) -> FruityResult<R>>
{
    fn from_script_value(value: ScriptValue) -> FruityResult<Self> {
        match value {
            ScriptValue::Callback(value) => Ok(Box::new(move |arg1, arg2, arg3| {
                let args: Vec<ScriptValue> = vec![
...
impl<
        ...
        R: TryIntoScriptValue + 'static,
    > TryIntoScriptValue for Box<dyn Send + Sync + Fn(T1, T2, T3) -> R>
{
    fn into_script_value(self) -> FruityResult<ScriptValue> {
