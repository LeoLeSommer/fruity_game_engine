...
/// `read`, e.g.:
#[derive(Debug)]
pub struct RwLock<T>(SendWrapper<std::cell::RefCell<T>>);

impl<T: Default> Default for RwLock<T> {
    fn default() -> Self {
        Self(SendWrapper::new(std::cell::RefCell::new(T::default())))
    }
}
...
impl<T> RwLock<T> {
    ...
    pub fn new(value: T) -> Self {
        Self(SendWrapper::new(std::cell::RefCell::new(value)))
    }
...
#[derive(Debug)]
pub struct RwLockReadGuard<'s, T>(std::cell::Ref<'s, T>);
...
#[derive(Debug)]
pub struct RwLockWriteGuard<'s, T>(std::cell::RefMut<'s, T>);
