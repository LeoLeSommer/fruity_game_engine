...
pub trait ScriptQueryParam: FruityAny {
...
pub struct ScriptQuery {
    ...
    on_entity_created: Signal<EntityReference>,
    on_entity_deleted: Signal<EntityId>,
...
impl ScriptQuery {
    ...
    #[export]
    pub fn for_each(&self, callback: Rc<dyn Fn(Vec<ScriptValue>) -> FruityResult<ScriptValue>>) {
        ...
        entities
            ...
            .try_for_each(|entity| {
                ...
                let script_params = script_params.into_iter().multi_cartesian_product();
    ...
    pub fn on_created(
        &self,
        callback: Rc<dyn Fn(Vec<ScriptValue>) -> FruityResult<Option<Rc<dyn Fn()>>>>,
    ...
    fn archetype_filter(&self) -> Box<dyn Fn(&ArchetypeArcRwLock) -> bool + 'static> {
        let params = self
            .params
            .iter()
            .map(|param| param.duplicate())
